...
The vale is packed with trees, and on a totem route it is easy to left-click the wrong one simply
because it sits in front of the tree you actually want. This plugin lets you pick the tree you are
chopping and clears the rest out of your way, so a stray click no longer drops you onto a maple you
never meant to touch.

## Screenshots

Before, the vale as the game draws it, with every tree and scenery pile a click target:

![Before: the cluttered vale](images/before.jpg)

After, with the hide toggles on and only the tree you are using left standing:
...
Pick the tree you are chopping. Two independent things then happen to everything else.
...
Trees outside the vale are left alone. The vale is identified by a hardcoded list of map region IDs,
not a global woodcutting override. Debug mode logs the regions you have loaded if you want to
...
3. Choose what happens to the other trees under **Trees you are not using**. "Walk here first"
   deprioritises their chop option, "Remove chop option" strips it, and "Leave alone" does nothing.
...
would race the map loader and tear the geometry. Objects pulled out only come back with a reload, so
any config change forces one via `GameState.LOADING`, and that means a short black screen.
